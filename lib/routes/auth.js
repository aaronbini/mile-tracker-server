...
app.post('/forgot', (req, res, next) => {
  ...
  cryptoPromise(20).then(token => token.toString('hex'))
    .then(token => ({ token, user: User.findOne({email}) })
    .then(tokenUser => {
      if (!tokenUser.user) {
        throw {status: 400, message: 'Invalid email or password.'};
      }
      tokenUser.user.resetPasswordToken = token;
      tokenUser.user.resetPasswordExpires = Date.now() + 3600000; // 1 hour
      return { token: tokenUser.token, user: user.save() }
    })
    .then(tokenUser => {
      //email logic
